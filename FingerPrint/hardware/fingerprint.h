...
typedef struct fingerprint_device {
    ...
    int (*self_test)(struct fingerprint_device *dev);

    int (*init)(struct fingerprint_device *dev);


    int (*set_spi_read)(struct fingerprint_device *dev,
                            int (*)(uint8_t reg, const uint8_t* data, uint32_t len));
    int (*set_spi_write)(struct fingerprint_device *dev,
                            int (*)(uint8_t reg, const uint8_t* data, uint32_t len));

    int (*spi_read)(uint8_t reg, const uint8_t* data, uint32_t len);
    int (*spi_write)(uint8_t reg, const uint8_t* data, uint32_t len);
